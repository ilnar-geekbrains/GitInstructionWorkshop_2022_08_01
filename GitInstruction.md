...
Базовая концепция git — ветки. Вы можете создать ответвление от основной ветки master в определенный момент времени и сохранять всю работу в своей отдельной «вселенной».

Когда работа будет завершена останется только слить вашу `git merge <name branch> `ветку с общей, предварительно перейдя на основную ветку master, и разрешить merge конфликты если такие будут. 
Влитые без ведома остальных членов команды изменения могут стать неожиданными. 
И вот тут появляется такая вещь как pull request и code review.
