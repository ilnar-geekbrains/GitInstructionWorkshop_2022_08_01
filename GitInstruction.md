...
для начала работы нужно представиться git, для этого прописываем в терминале команды:
...
В проводнике Visual Studio создаем папку, либо через файл-открыть открываем имеющуюся папку

командой терминала VS:

git status проверяем в нужной ли папке мы находимся

git init - инициализируем репозиторий в текущей папке

git add <имя файла> добавление файла в "отслеживаемые" GITом.

git commit -m - фиксация изменений в файлах репозитория.
