...
git clone - копируем репозиторий, который находится на сервисе GitHub в наш локальный репозиторий.
2. Проверяем директорию в какой находимся, если требуется сменить, то выполняем команду cd.
3. Создаём ветку git branch.
